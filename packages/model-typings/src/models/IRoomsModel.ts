...
export interface IRoomsModel extends IBaseModel<IRoom> {
	...
	setUdeskAssignDataById(rid: string, udeskAssign: any): Promise<UpdateResult>;
	findOneById(roomId: string, options: any): any;
	setAnnouncementReadById(_id: string, announcementId: string, username: string): Promise<UpdateResult>;
	findBySubscriptionUserIdAndTypeUpdatedAfter(userId: string, updatedAfter: Date, types: any, options?: FindOptions<IRoom>): any;
	findBySubscriptionUserIdAndTypes(userId: string, types: any, options?: FindOptions<IRoom>): any;
	setValuePropositionById(rid: string, roomValuePropositionInfo: any): any;
	setAppiaRoomTypeById(rid: string, type: IRoom['t']): Promise<UpdateResult>;
	setFederatedStatus(roomId: any, status: boolean): any;
}
