...
const closePopup = function (res) {
	...
	const content = `<html><head><script>if(window.opener) {window.close();} else {window.location.href='/home';}</script></head></html>`;
...
const casTicket = function (req, token, callback) {
	...
	if (!settings.get('CAS_enabled')) {
		...
		console.log('Got ticket validation request, but CAS is not enabled');
		callback();
	}
