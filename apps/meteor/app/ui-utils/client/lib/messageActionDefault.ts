...
Meteor.startup(async function () {
	...
	MessageAction.addButton({
		...
		async action(_, props) {
			...
			if ($msg) {
				const msgContent = $msg.innerHTML; // 获取 innerHTML 的字符串
				message.editedMessage = msgContent.replace(/<div class=['"]edit-message-mark['"][^>]*>.*?<\/div>/, '');
				message.editedMessage = msgContent.replace(/<div class=['"]avatar-name['"][^>]*>.*?<\/div>/, '');
				console.info('editedMessage=============', message.editedMessage);
			}
			await chat?.composer?.dismissAllQuotedMessages();
			await chat?.messageEditing?.editMessage(message);
		},
		condition({ message, subscription }) {
			return !!subscription && Meteor.user()?.username === message?.u?.username;
		},
		order: 1,
		group: ['message', 'menu'],
	});
...
*/
	...
	MessageAction.addButton({
		...
		condition({ message, subscription }) {
			return (!message.msgType || message.msgType === 'forwardMergeMessage') && !!subscription;
		},
		order: 8,
		group: 'menu',
	});
