...
export const sendAPN = ({ userToken, notification, _removeToken }) => {
	...
	console.log('[PUSH] to apnConnection.send');

	apnConnection.send(note, userToken).then((response) => {
		console.log('[PUSH] apnConnection.send  response:', response);
		response.failed.forEach((failure) => {
			logger.debug(`[PUSH] Got error code ${failure.status} for token ${userToken}`);
			...
			try {
				AppiaLog.insertData(
					...
					'pushError',
					{ userToken },
					{ failure, code: failure.status, notification, note, response },
				).then((r) => {
					console.log('AppiaLog.insertData then:', r);
				});
			} catch (e) {
				console.log('AppiaLog.insertData Failed:', e.message);
			}
