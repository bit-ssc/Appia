...
export const sendNotification = async ({
	...
	if (
		...
		const sendData = await getPushData({
			notificationMessage,
			room,
			message,
			userId: subscription.u._id,
		...
		console.log('sendNotificationsOnMessage.js  sendData:', sendData);
		queueItems.push({
			type: 'push',
			data: sendData,
		});
...
export async function sendMessageNotifications(message, room, usersInThread = []) {
	...
	if (mentionIds.length > 0) {
		await Promise.all(
			mentionIds.map(async (mentionId) => {
				const mentionSubscription = await Subscriptions.findOneByRoomIdAndUserId(room._id, mentionId);
				subscriptions.push(mentionSubscription);
			}),
		);
	}
	const map = {};
	const arr = Array.from(
		new Set(
			subscriptions.map((subscription) => {
				map[subscription.u._id] = subscription;
	...
	console.info('sendMessageNotifications subscriptions', subscriptions);
	arr.forEach((id) => {
		const subscription = map[id];
