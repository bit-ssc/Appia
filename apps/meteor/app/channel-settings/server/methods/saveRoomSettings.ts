...
import { saveRoomAnnouncement, saveRoomAnnouncementRead } from '../functions/saveRoomAnnouncement';
...
import { saveFederated } from '../functions/saveFederated';
import { saveAppiaRoomType } from '../functions/saveAppiaRoomType';
...
import { saveRoomValueProposition } from '../functions/saveRoomValueProposition';
import { roomCoordinator } from '../../../../server/lib/rooms/roomCoordinator';
import { RoomSettingsEnum } from '../../../../definition/IRoomTypeConfig';
import { getOrgMatrixDomain } from '../../../api/server/lib/external-channel-common';
...
const settingSavers: RoomSettingsSavers = {
	...
	async roomValueProposition({ value, room, rid, user }) {
		const message = (room.valueProposition && room.valueProposition.message) || '';
		if (value !== message) {
			await saveRoomValueProposition(rid, value, user, room);
		}
	},
	async roomAnnouncement({ value, room, rid, user }) {
		const message = (room.announcement && room.announcement.message) || '';
		if (value !== message) {
			await saveRoomAnnouncement(rid, value, user);
		}
	},
	async roomAnnouncementRead({ value, room, rid, user }) {
		const id = (room.announcement && room.announcement._id) || undefined;
		if (value === id) {
			await saveRoomAnnouncementRead(rid, value, user);
		}
	},
	...
	async appiaRoomType({ value, room, rid, user }) {
		if (value === room.rt) {
			return;
		}
		await saveAppiaRoomType(rid, value, user);
	},
	async streamingOptions({ value, rid }) {
		await saveStreamingOptions(rid, value);
	},
	async federated({ value, room, rid, user }) {
		if (value === room.federated) {
			return;
		}
		await saveFederated(rid, value, user);
	},
...
async function saveRoomSettings(
	rid: IRoom['_id'],
	originSettings: Partial<RoomSettings> | keyof RoomSettings,
	value?: RoomSettings[keyof RoomSettings],
): Promise<{ result: true; rid: IRoom['_id'] }> {
	let uid = Meteor.userId();

	let { settingUsername, settingOrg, ...settings } = originSettings;
	console.log(`saveRoomSettings settingUsername:${settingUsername} settingOrg:${settingOrg}`);
	let externalUser;
	let externalUsername;
	if (!uid) {
		if (settingUsername.includes(':')) {
			settingUsername = settingUsername.split(':')[0];
		}
		if (process.env.COMPANY.toLowerCase() !== settingOrg.toLowerCase()) {
			const domain = getOrgMatrixDomain(settingOrg);
			externalUsername = `${settingUsername}:${domain}`;
			console.log(`saveRoomSettings externalUsername:${externalUsername}`);
		}
		externalUser = await Users.findOneByUsername(externalUsername);
		if (externalUser) {
			console.log('saveRoomSettings externalUser:', externalUser);
			uid = externalUser._id;
		}
	}
	...
	const user = ((await Meteor.userAsync()) as IUser & Required<Pick<IUser, 'username' | 'name'>>) || externalUser;
	if (!user) {
		throw new Meteor.Error('error-invalid-user', 'Invalid user', {
			method: 'saveRoomSettings',
		});
