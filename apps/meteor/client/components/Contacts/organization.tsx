...
const UserComponent: React.FC<{ user: IStaff; left: number; showDepartment: boolean }> = ({ user, left, showDepartment = true }) => {
	...
	const { username, name, departmentNames } = user;
	let departmentName = '';

	if (departmentNames?.length) {
		departmentName = departmentNames[0].split('/').pop();
	}

	const onClick = () => {
		console.log('dxd========user', user, disabled, selected);
	...
	const renderLastItem = () => {
		return (
			...
			>
				...
				<span>分享链接至{companyKV[org].shortName}合作伙伴群</span>
			</div>
		);
	};
...
const PartnersComponent = () => {
	...
	const renderGuide = () => {
		...
		const renderDot = (content: string) => <div style={{ background: '#1677ff', borderRadius: '50%', color: 'white', height: 16, width: 16,  padding: 2, fontSize: 12 }}>{content}</div>;
	...
	const renderList = () => (
		<div style={{ width: '100%' }}>
			<div style={{ marginBottom: 8, fontWeight: '600', width: '100%' }}>{t('Partner_Guide_title')}</div>
			...
			<div className={classNames('content', keyword && 'hidden')}>
				<Box className={tabsStyle}>
					...
					<div className={activeTab === 2 ? 'active tab' : 'tab'} onClick={() => setActiveTab(2)}>
						{t('Appia_Contact_info_add_disc_member')}
					</div>
