...
const MessageHeader = ({ message, context, uploading, showActionMenu }: MessageHeaderProps): ReactElement => {
	...
	let resultName = roomName || message.alias || getUserDisplayName(user.name, user.username, true) || '';
	if (resultName.length === 2) {
		resultName = resultName.split('').join('\u3000');
	}

	return (
		<FuselageMessageHeader>
			<MessageNameContainer
				style={{ width: '43px', textAlign: 'justify', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
			>
				<MessageName
					className='rcx-message-header-name'
...
=======
			{/* {!['search', 'mentions'].includes(context || '') && message.msgType !== 'oncall' && !message.private && !uploading && (
>>>>>>> a8c77c9e35 (Merge branch 'zfc/0129_line' into 'prd/250129')
