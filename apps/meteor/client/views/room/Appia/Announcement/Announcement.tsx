...
import { Modal } from '../../../../components/AppiaUI';
...
const Announcement: FC<AnnouncementParams> = ({ announcement, room }) => {
	...
	const handleDelete = (_id: string) => {
		Modal.confirm({
			title: t('Announcement_Delete_Toast'),
			icon: <ExclamationCircleOutlined />,
			okText: t('Confirm'),
			cancelText: t('Cancel'),
			onOk: () => deleteAnnouncement(_id),
		});
	};
	...
	const renderAnnouncementHeader = (announcement: IRoomAnnouncement) => {
		// meeting
	...
	return dataSource.length > 0 || canEdit ? (
		<AnnouncementComponent>
			<div className='antHeader' ref={elementRef}>
				<div className='antTitle1'>{t('Announcement')}</div>
				{dataSource.length > 1 ? (
					<div className='antRow' onClick={() => setIsAnnouncementOpen(false)}>
						<VerticalBarClose />
					</div>
				) : null}
			</div>
												...
												<Box key={element.fileUrl} display='flex' alignItems='center' style={{ cursor: 'pointer', marginRight: '10px' }}>
													...
													<Box
														...
														onClick={() => {
															setModal(
																<FilePreview
																	url={getURL(element.fileUrl)}
																	fileName={element.fileName}
																	fileSize={1024}
																	onClose={closeFlieModal}
																/>,
															);
								...
								{canEdit ? (
									<Box style={{ width: '48px', display: 'flex', flexDirection: 'row' }}>
										<div
											...
											onClick={() => {
												console.log('dxd=======item', item);
												console.log('dxd=========announcement', announcement);
												if (
													// 编辑会议
												...
												setModal(<Edit announcement={item} rid={room._id} onClose={closeFlieModal} />);
											}}
										>
											<EditIcon />
										</div>
										<div style={{ marginLeft: 16, cursor: 'pointer' }} onClick={() => handleDelete(item._id)}>
											<DeleteIcon />
										</div>
									</Box>
								) : null}
