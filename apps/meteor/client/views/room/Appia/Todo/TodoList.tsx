...
const TodoList: FC<AnnouncementParams> = ({ room }) => {
	...
	const goToMessage = (todo: IToDo) => {
		...
		if (message?.t === 'rollback-message') {
			dispatchToastMessage({ type: 'error', message: t('Message_Recalled') });
		}
	};
													...
													<Box
														...
														onClick={(e) => {
															...
															if (element.title_link) {
																const isImage = /\.(jpg|jpeg|png|gif|webp|bmp)$/i.test(element.title || '');
																if (isImage) {
																	openImage(getURL(element.title_link));
																} else {
