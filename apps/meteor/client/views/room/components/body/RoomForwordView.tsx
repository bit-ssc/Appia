...
const RoomForwordView = memo(({ selectedMessageCount, selectedMessageStore, prid, room }: IRoomForwordViewProps) => {
	...
	const flattenMDValues = (values: any[], isTopLevel: boolean = true, listType?: string): string => {
		let result = '';
		values.forEach((item, index) => {
			...
			if (item.type === 'LIST_ITEM') {
				...
				if (listType === 'UNORDERED_LIST') {
					result += '- ';
				} else if (listType === 'ORDERED_LIST' && item.number) {
					result += `${item.number}. `;
				}
			}
			...
			if (typeof item === 'string') {
				...
				result += flattenMDValues(item, false, listType);
				...
				// 处理链接
					...
					result += flattenMDValues(item.value.label, false);
				}
				// 处理有序列表和无序列表
				else if (item.type === 'UNORDERED_LIST' || item.type === 'ORDERED_LIST') {
					result += flattenMDValues(item.value, false, item.type);
				}
				// 如果存在value属性，并且value是数组，则递归
				else if (Array.isArray(item.value)) {
					result += flattenMDValues(item.value, false, listType);
			...
			if (isTopLevel && index < values.length - 1) {
				// 顶层元素之间的换行
				result += '\n';
			} else if (!isTopLevel && listType && item.type === 'LIST_ITEM' && index < values.length - 1) {
				// 列表项之间的换行
				result += '\n';
			}
		});
	...
	const handleSetAsAnnouncement = async () => {
		...
		try {
			...
			for (const msg of messages) {
				...
				const flatText = normalizedMessage.md ? flattenMDValues(normalizedMessage.md) : '';
			...
			for (const messageId of messageIds) {
				...
				if (message) {
					...
					if ($msg) {
						...
						messagesCache[messageId]._htmlContent = $msg.innerHTML.replace(/<div class=['"]edit-message-mark['"][^>]*>.*?<\/div>/, '');
	...
	const saveMessageContentFromCache = (messageId: string, cachedMessage: any) => {
		...
		const hasFile = cachedMessage.hasOwnProperty('file') || 
