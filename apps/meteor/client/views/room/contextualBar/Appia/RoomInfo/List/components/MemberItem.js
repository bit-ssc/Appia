...
export const MemberItem = ({
	...
	const { emtMembers } = useContactContext();
	...
	if (isTeam && roleMap.owner) {
		content = (
			<>
				<RPIcon fontSize={12} style={{ paddingTop: 3 }}/> {t('role_name_owner1')}
			</>
		);
	} else if (roleMap.owner) {
		content = (
			<>
				<RPIcon fontSize={12} style={{ paddingTop: 3 }}/> {t('role_name_owner2')}
			</>
		);
	} else if (isTeam && roleMap.moderator) {
		content = (
			<>
				<Icon name='shield-alt' size='x12' style={{ marginTop: -2 }}></Icon> {t('role_name_moderator1')}
			</>
		);
	} else if (roleMap.moderator) {
		content = (
			<>
				<Icon name='shield-alt' size='x12' style={{ marginTop: -2 }}></Icon> {t('role_name_moderator2')}
			</>
		);
	} else if (roleMap['ghost-owner']) {
		content = (
			<>
				<Icon name='shield-alt' size='x12' style={{ marginTop: -2 }}></Icon> {t('role_name_owner_ghost')}
			</>
		);
	}
	...
	const getRank = () => {
		...
		const isEMT = emtMembers.some((member) => member.username === username);
		if (isEMT) {
			return 'EMT';
		}
