...
import { useTranslation, usePermission, useSetModal } from '@rocket.chat/ui-contexts';
import React, { useState, useEffect } from 'react';
import type { FC } from 'react';

import { AnnouncementPanelIcon, ArrowIcon } from '../../../components/AppiaIcon';
		...
		<div className='room-panel-container'>
			<div className='room-panel-header'>
				...
				<div style={{ display: 'flex', alignItems: 'center' }}>
					{canEdit && <CreateAnnouncement room={room} />}
					{announcements && announcements.length > 0 && (
						<>
							<ArrowIcon
								fontSize='16px'
								style={{ transform: `rotate(${!isAnnouncementOpen ? 0 : 180}deg)`, transition: 'transform 0.5s ease' }}
							/>
							<div
							...
							>
								{`${isAnnouncementOpen ? t('fold') : t('unfold')}(${announcements?.length})`}
							</div>
						</>
					)}
				</div>
