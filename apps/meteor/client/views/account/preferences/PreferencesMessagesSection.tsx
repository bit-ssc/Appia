...
const PreferencesMessagesSection = ({ onChange, commitRef, ...props }: FormSectionProps): ReactElement => {
	...
	const settings = {
		...
		showImageSummary: useUserPreference('showImageSummary') ?? true,
		showDocumentSummary: useUserPreference('showDocumentSummary') ?? true,
	};
	...
	const {
		...
		handleClockMode,
		handleSendOnEnter,
		handleShowImageSummary,
		handleShowDocumentSummary,
	} = handlers;
		...
		<Accordion.Item title={t('Messages')} {...props}>
			<FieldGroup>
				...
				{useMemo(
					() => (
						<Field display='flex' flexDirection='row' justifyContent='spaceBetween' flexGrow={1}>
							<Field.Label>{t('Show_Message_Image_Summary')}</Field.Label>
							<Field.Row>
								<ToggleSwitch checked={showImageSummary} onChange={handleShowImageSummary} />
							</Field.Row>
						</Field>
					),
					[handleShowImageSummary, t, showImageSummary],
				)}
				{useMemo(
					() => (
						<Field display='flex' flexDirection='row' justifyContent='spaceBetween' flexGrow={1}>
							<Field.Label>{t('Show_Message_Doc_Summary')}</Field.Label>
							<Field.Row>
								<ToggleSwitch checked={showDocumentSummary} onChange={handleShowDocumentSummary} />
							</Field.Row>
						</Field>
					),
					[handleShowDocumentSummary, t, showDocumentSummary],
				)}
				{/**
				{useMemo(
					() => (
