...
export const process = async (chat: ChatAPI, message: IMessage): Promise<void> => {
	...
	await call('sendMessage', message)
		.then(() => ChatMessage.update(message._id, { $set: { sendFailed: false, temp: false } }))
		.catch(() => {
			ChatMessage.insertOrUpdateErrorMessage(message);
			chat.composer?.dismissAllQuotedMessages();
		});
};

export const sendMessage = async (chat: ChatAPI, { text, tshow, md }: { text: string; tshow?: boolean, md: unknown }): Promise<boolean> => {
