...
export type ComposerAPI = {
	...
	replaceText(
		...
		reduceSelectionStart?: boolean,
	): void;

	setCursorToEnd(): void;
	setCursorToStart(): void;
	updateCursorStart(start: number): void;
	...
	dismissEditedMessage(mid: IMessage['_id']): Promise<void>;
	dismissAllEditedMessages(): Promise<void>;
	readonly quotedMessages: Subscribable<IMessage[]>;
	readonly editedMessages: Subscribable<IMessage[]>;
...
export type DataAPI = {
	...
	recallMessage(mid: IMessage['_id']): Promise<void>;
...
export type UploadsAPI = {
	...
	send(
		file: File,
		{ description, msg }: { description?: string; msg?: string; fileId?: string; fileName?: string; md: unknown },
	): Promise<void>;
};
...
export type ChatAPI = {
	...
	readonly userCard: {
		open(username: string, roomSender?: any): (event: UIEvent) => void;
		close(): void;
	};
	...
	readonly flows: {
		readonly uploadFiles: (files: readonly File[], resend?: boolean) => Promise<void>;
		readonly sendMessage: ({ text, tshow, md }: { text: string; tshow?: boolean; md: unknown }) => Promise<boolean>;
		...
		readonly setMessageTodos: (params: { messageId: string; status: number; tips?: string; type?: 'd' | 'h' }) => Promise<void>;
		readonly finishMessageTodo: (params: { id: string; status: number }) => Promise<void>;
		readonly resendFederationMessage: (message: IMessage) => Promise<void>;
	};
};
